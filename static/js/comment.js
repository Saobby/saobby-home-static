...
function read_comment(e, t) {
    ...
    for (n in d += `<div style="position:relative;left:${20 * t}px;" id="comment-area-${e.cid}"><div style="border-bottom: 2px solid #ddd;padding:12px 0px;"><img src="${e.avatar_url}" alt="用户头像" width="32px" height="32px"><b style="position:relative;top:-17px;left:5px;">${e.nickname ? rsc(e.nickname) : c}</b><span style="color:#777777;position:relative;top:-17px;left:5px;" class="middle"> <img src="/static/image/icon/mail-check-grey.svg" width="16px" height="16px" alt="(已绑定电子邮箱)" title="此用户已绑定电子邮箱" class="middle" ${e.is_email_checked?"":"hidden"}> <img src="/static/image/icon/pencil-check-grey.svg" width="16px" height="16px" alt="(已编辑)" title="(已编辑)" class="middle" ${e.modify_time?"":"hidden"}> <img src="/static/image/icon/clock-grey.svg" width="16px" height="16px" alt="发表时间" class="middle">${e.modify_time ? ts2str(e.modify_time) : ts2str(e.timestamp)} #${e.cid}</span><br>`,
