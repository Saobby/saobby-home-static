...
function read_comment(e, t) {
    var n, d = "", i = e.content, o = marked.parse(i), c = e.username.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;"), l = e.reply_to, i = i.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;");
    for (n in d += `<div style="position:relative;left:${40 * t}px;" id="comment-area-${e.cid}"><div style="border-bottom: 2px solid #ddd;padding:12px 16px;"><img src="${e.avatar_url}" width="32px" height="32px"><b style="position:relative;top:-17px;left:5px;">${e.nickname ? e.nickname.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;") : c}</b><span style="color:#777777;position:relative;top:-17px;left:5px;"> ${e.modify_time ? "编辑于" : "发表于"} ${e.modify_time ? ts2str(e.modify_time) : ts2str(e.timestamp)} #${e.cid}</span><br>`,
...
mdr.image = function(e, t, n) {
    var d = Math.random();
    return `<img src_="${e}" alt="${n}" title="${t || ""}" class="marked-img" id="marked-img-${d}" hidden><a onclick="show_image(gebi('marked-img-${d}'));this.hidden=true;" href="javascript:;">点击加载外部图片</a>`
}
,
