...
function get_element_abs_pos2(e) { for (var t = e.offsetTop, n = e.offsetLeft; e = e.offsetParent;) t += e.offsetTop, n += e.offsetLeft; return { left: n, top: t } }
...
function add_root_comment() { var e = document.getElementById("content").value; "" != e ? (get_captcha_img(), document.getElementById("captcha-window").hidden = !1, args = { place_id: place_id, access_token: localStorage["access-token"], content: e, reply_to: -1 }) : document.getElementById("result0").innerHTML = "评论不能为空!" }

function complete_captcha() {
  -1 == args.reply_to ? (document.getElementById("rel-btn").disabled = !0, document.getElementById("rel-btn").innerHTML = "请稍候") : (document.getElementById(`rel-btn-${args.reply_to}`).disabled = !0, document.getElementById(`rel-btn-${args.reply_to}`).innerHTML = "请稍候");
...
function get_all_comment() {
  ...
  var t = { place_id: place_id, amount_per_page: 8, page_index: comment_page_index, access_token: localStorage["access-token"], scroll_to: scroll_to };
  e.send(JSON.stringify(t)), e.onreadystatechange = function() {
    if (4 == e.readyState) {
      ...
      if (t.success) {
        ...
        var a = "";
        1 != o && (a += '<button onclick="change_page(0);">«</button>');
...
function save_edition(e) {
  ...
  if ("" !== t)
    if (t !== document.getElementById(`origin-content-${e}`).value) {
      ...
      n.open("POST", domain + "/api/edit_comment", !0), n.setRequestHeader("Content-Type", "application/json"), n.send(JSON.stringify({ access_token: localStorage["access-token"], cid: e, content: t })), n.onreadystatechange = function() {
        if (4 == n.readyState) {
...
function add_reply_comment(e, t) { "" != t ? (get_captcha_img(), document.getElementById("captcha-window").hidden = !1, args = { place_id: place_id, access_token: localStorage["access-token"], content: t, reply_to: e }) : document.getElementById(`reply-result-${e}`).innerHTML = "评论不能为空!" }
...
function show_image(e) { e.src = e.getAttribute("src_"), e.hidden = !1 } String.prototype.replaceAll = function(e, t) { return this.replace(new RegExp(e, "gm"), t) };
var comment_page_index = 0,
  nurl = window.location.href.split("?"),
  place_id = 114514;
if (2 === nurl.length) {
  var args = nurl[1].split("&");
  place_id = args[0], is_in_array(args, "comments-only") && (document.getElementById("contact").hidden = !0, document.getElementById("tools").hidden = !0);
