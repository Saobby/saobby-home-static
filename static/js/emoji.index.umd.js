      ...
      extensions: [{
        ...
        renderer(token) {
          if (options.unicode) {
            ...
            return `<img style="vertical-align:text-bottom;" height="${token.name.startsWith('stk_')?'64px':'24px'}" width="${token.name.startsWith('stk_')?'64px':'24px'}" alt="${token.name}" src="${token.emoji}"${this.parser.options.xhtml ? ' /' : ''}>`;
          }
        }
      }]
    };
