...
async function load_webcounters(page_index) {
    ...
    for (let i = 0; i < rsp.data.counters.length; i++){
        ...
        html += `<div class="bottom-line"><b style="font-size:20px;">${counter.desc?counter.desc:counter.counter_id}</b><br><span style="color:#777;">${icon_with_text("clock-grey", ts2str(counter.create_at))}</span><br><a href="/webcounter_dashboard?access_token=${counter.access_token}" target="_blank"><button class="wux-btn wux-btn-primary wux-btn-sm">${icon_with_text("external-link-white", "进入控制面板")}</button></a></div>`;
    }
    gebi("webcounters").innerHTML = html;
    gen_cp_buttons(rsp.data.page_index+1, rsp.data.page_amount, 8, (n)=>{load_webcounters(n-1).then();}, gebi("change-page-div"), "wux-btn cp-btn", "wux-btn wux-btn-outline cp-btn");
