    ...
    fetch_data(domain+"/api/get_posts_list", "POST", headers, JSON.stringify(send_data)).then(function(val){
        ...
        if (rep.success){
            ...
            for (var i in rep.data.posts){
                ...
                pl_html += `<div class="bottom-line"><a href="/post/?pid=${post.name}" target="_blank"><b style="font-size:20px"><span style="color:#5064E1;">${post.is_pinned?'<img src="/static/image/icon/pinned-primary.svg" width="24px" alt="[置顶]" class="middle">':""}</span><span style="color:#${post.is_closed?"AA0000":"00AA00"};">${post.is_closed?'<img src="/static/image/icon/lock-red.svg" width="24px" alt="[已关闭]" class="middle">':'<img src="/static/image/icon/circle-dot-green.svg" width="24px" alt="[开放]" class="middle">'}</span><span class="post-list-title">${post.title}</span></b></a>${tags_html}<br><span class="middle"><img src="/static/image/icon/user.svg" width="20px" alt="作者" class="middle">${post.author}</span><span class="gray"> <img src="/static/image/icon/clock-grey.svg" width="20px" alt="发布时间" class="middle">${ts2str(post.modify_time)} <img src="/static/image/icon/thumb-up-grey.svg" width="20px" alt="点赞数" class="middle">${post.loves} <img src="/static/image/icon/eye-grey.svg" width="20px" alt="访问数" class="middle">${post.views}</span></div>`;
            }
            gebi("posts-list-html").innerHTML = pl_html?pl_html:`<span class="gray">没有帖子</span>`;
